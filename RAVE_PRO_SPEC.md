...
- The same quote fetcher (`ensureQuote`/`getQuote`/`sparklineSvg`) is reused by the Dashboard's HUD strip: the strip's four cells are Today's P&L, Trades Today, a live VIX reading + sparkline (`vixHudCell()` — replaced a duplicate on-device Rule Adherence % that was already shown in full on the Coach tab), and Max Drawdown (`maxDrawdownStats()`, mark-to-market where MAE data is available — see §6 — replaced a daily-loss progress bar that moved into the risk-guardrail alert banner). Max Drawdown's HUD value is rendered in neutral text rather than loss-red — it's a standing risk figure you look at constantly, not a one-off bad outcome, so it doesn't get the same alarm treatment as a losing trade.
- A third consumer of the same quote fetcher: a Bloomberg-terminal-style scrolling ticker strip (`renderTicker()`, `.ticker-strip`/`.ticker-track` in `index.html`), pinned under the Equity Pulse bar on every tab, not just Watchlist/Dashboard. It shows the major futures roots (ES, NQ, YM, RTY, GC, CL) plus VIX plus every symbol on your Watchlist, each with live price and change, auto-scrolling via a pure-CSS marquee (two duplicated copies of the item list, translated by exactly one copy's width, so the loop has no visible seam) — pauses on hover so it's actually readable, and falls back to a static scrollable row under `prefers-reduced-motion`. Purely cosmetic beyond the shared data source: no separate network path, no new state.
...
A balance-forward table (`accountsTable()`) is the primary view: one row per account with status, running balance, today's P&L, net P&L, drawdown buffer, and trade count, plus a combined-balance/today total in the header — built to make "how many accounts do I have and what's each one worth" answerable at a glance. Below it, `accountsEquityChart()` plots cumulative P&L for every prop account with trades only (one line per account, fixed categorical color order, colorblind-validated for dark mode), each sequence-indexed like the Dashboard equity curve, with direct end-of-line labels (account name + net P&L). Personal/untagged trades are deliberately excluded from this chart — an earlier version also plotted a neutral dashed line for them, but Accounts is scoped to prop-firm accounts specifically, not a mixed view of personal + funded trading; personal equity still shows on the main Dashboard. This replaced an earlier stat-grid "Fleet Summary" panel that duplicated numbers already visible in the table and per-account cards. Every account card below it still shows its own rules and meters, unchanged.

### Component: Payout & Milestone Roadmap (per account, shown for `funded`/`passed` accounts)
- Buffer above max drawdown: `maxDrawdown − trailingUsed` (or vs static balance if `ddType === 'static'`), shown as $ and %. `trailingUsed` is where the account stands right now (current distance below its peak); a separate `trailingWorstEver` — the worst mark-to-market drawdown it ever reached, even if since recovered — is what actually determines a breach. See §6 for why these are two different numbers.
...
### Tier 2 — Real LLM chat (beta, opt-in, uses your own Anthropic API key)
An "AI Coach" panel in Coach is a genuine back-and-forth conversation with Claude, not a one-shot report. It started as a single "Run AI Audit" button that returned one static write-up; it's now a chat thread (`state.aiChat.thread`, a UI-only runtime array — never persisted, matching the rest of this tier's privacy posture) that you can ask follow-up questions in, exactly like the on-device Coach chat below it visually, but backed by a real model instead of local pattern matching.

Mechanically: `sendAiChat(question)` appends the new turn to the thread, then re-sends the **entire thread so far** as `messages` on every call, because Anthropic's Messages API is stateless per-request — there's no server-side session to remember previous turns, so the full conversation has to travel with every call. A `system` prompt is rebuilt fresh on each send from the current trade log (most recent 20 trades) and trading plan, so context stays current even if you log a new trade mid-conversation. Three suggested opening questions (one of which reproduces the original single-shot audit prompt) seed the empty state; after that it's a free-form textbox, same interaction pattern as the local Coach chat.
...
// state.aiChat = { status: 'idle'|'loading'|'error', error: '', thread: [{role, text}] } — runtime only, never persisted
```

**Caveats to know before using it:** this sends trade data (symbol, direction, size, P&L, tags, notes) to Anthropic's API using your key and their usage terms apply, and every message in a growing conversation re-sends the whole thread, so token cost (and therefore your own API cost) grows with the length of the chat, not just the latest message. Anthropic's direct-browser-access mode is intended for exactly this kind of personal/local tool, not production apps with other users, which fits RAVE's single-user model — but it does mean your key is visible to anyone with access to this browser/device. Don't use a key that's shared with anything sensitive. The thread is not persisted, so it resets on reload — a deliberate choice to avoid quietly accumulating a long-lived chat transcript with a third party in local storage without you opting into that separately.
...
### Mark-to-Market Max Drawdown — **SHIPPED**, part of this section
The original Max Drawdown calc only walked the cumulative P&L curve at trade close — a trade that dove hard against you intra-trade and then recovered by the time you closed it was invisible to that number, even though a real prop-firm risk system (and TradingView's strategy tester) marks equity to market continuously and would have counted it. This under-counts real drawdown risk, which is the whole point of the metric.

`maxDrawdownStats(sorted)` fixes this using the trade-level `mae` field (Maximum Adverse Excursion — "worst point during the trade, in $" — already collected in the trade form, previously unused by any calculation): for each trade with `mae` logged, it checks the equity low reached *while that trade was still open* (peak-so-far minus that trade's `mae`) against the running peak, in addition to the usual peak-vs-close check. Trades without `mae` fall back to the old closed-trade-only treatment, so the calc degrades gracefully on partial data rather than breaking. The result carries `usedMae` and `maeCoverage` (0–1) so the UI can be honest about which mode is in effect — the Dashboard HUD's Max Drawdown cell shows a small "mark-to-market" / "mark-to-market, partial data" / "closed trades only" note accordingly.

The same helper now backs `accountStats()` (§2's per-account drawdown-limit tracking), split into two distinct numbers: `trailingUsed` (current distance below peak — what a live "how close am I to the limit" gauge should show) and `trailingWorstEver` (the worst mark-to-market drawdown the account ever reached, even if it has since recovered — what actually determines `ddBreached`). A firm's drawdown limit is a hard stop the moment it's touched; showing only the current, recovered-since number would tell a trader they're fine when their account would already have been shut down. When a breach was specifically a mark-to-market event invisible in closed P&L, the account's alert banner says so explicitly.

Validated against a real TradingView strategy export (33 ES trades, March–August): the mark-to-market calc reproduces TradingView's reported max drawdown exactly ($2,587.50 vs. the old closed-trade-only $1,825).
